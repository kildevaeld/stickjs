...
function controller(controllerName) {
    return function (target) {
        let name = controllerName || utilities_1.camelcase(target.name);
...
function module(moduleName) {
    return function (target) {
        let name = moduleName || utilities_1.camelcase(target.name);
...
function service(serviceName, moduleName) {
    return function (target) {
        let name = serviceName || utilities_1.camelcase(target.name);
